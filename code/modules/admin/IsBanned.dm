	...
	*/ // NOVA EDIT REMOVAL END
	...
	if (!real_bans_only && !.)
		if (SSplexora.enabled && CONFIG_GET(flag/require_discord_verification))
			if (CONFIG_GET(flag/plexora_verification_whitelist_staff) && (admin || mentor))
				log_access("Allowed entry: [ckey] is a(n) [admin ? "admin" : mentor ? "mentor" : "???"] and bypassed Discord verification.")
			else

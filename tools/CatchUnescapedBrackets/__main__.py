...
def find_lone_arrays(src: str):
  ...
  at_string = False  # Track if the string is prefixed with @
  ...
  while i < length:
    ...
    if ch == '"':
      if not in_string:
        # Check if the string is prefixed with @
        at_string = i > 0 and src[i - 1] == '@'
      in_string = not in_string
      i += 1
      continue

    # illegal []
    if in_string and not at_string and ch == '[' and nxt == ']':
